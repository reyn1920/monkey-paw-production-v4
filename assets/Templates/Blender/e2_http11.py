...
class Request:
    ...
    def parse(
        ...
        # to implement HTTP/1.1 strictly, there's little need for lenient
        # parsing.
        ...
        if method != b"GET":
            raise ValueError(
                f"unsupported HTTP method; expected GET; got {
                    d(method)}"
            )
...
class Response:
    ...
    def parse(
        ...
        try:
            ...
            raise ValueError(
                f"invalid status code; expected integer; got {
                    d(raw_status_code)}"
            ) from None
        if not 100 <= status_code < 600:
            raise ValueError(
                f"invalid status code; expected 100–599; got {
                    d(raw_status_code)}"
            )
    ...
    # Conversions from str to int are protected by sys.set_int_max_str_digits..
        ...
        while True:
            ...
            if len(body) + chunk_size > MAX_BODY_SIZE:
                raise SecurityError(
                    f"chunk too large: {chunk_size} bytes after {
                        len(body)} bytes"
                )
