...
class _ModelNamespaceDict(dict):
    ...
    def __setitem__(self, k: str, v: object) -> None:
        ...
        if (
            ...
            warnings.warn(
                f"`{k}` overrides an existing Pydantic `{
                    existing.decorator_info.decorator_repr}` decorator"
            )
        ...
        if bases:
            ...
            if private_attributes or base_private_attributes:
                ...
                if original_model_post_init is not None:
                    # if there are private_attributes and a model_post_init
            ...
            # Use the getattr below to grab the __parameters__ from the
            # `typing.Generic` parent class
            ...
            # These are instance variables, but have been assigned to
            # `NoInitField` to trick the type checker.
            for instance_slot in (
                ...
                namespace.pop(
                    instance_slot,
                    None,
                    # In case the metaclass is used with a class other than
                    # `BaseModel`.
                )
        ...
        if var_name == "model_config" or var_name == "__pydantic_extra__":
            ...
            if var_name.startswith("__"):
                ...
                raise NameError(
                    ...
                    f' use sunder names, e.g. {
                        "_" +
                        var_name!r} instead of {
                        var_name!r}.'
                )
            ...
            if var_name in base_class_fields:
                ...
                raise PydanticUserError(
                    f"Field {
                        var_name!r} requires a type annotation",
...
def set_default_hash_func(cls: type[BaseModel], bases: tuple[type[Any], ...]) -> None:
    ...
    if (
        ...
        # In the last case we still need a new hash function to account for new
        # `model_fields`.
...
def make_hash_func(cls: type[BaseModel]) -> Any:
    ...
    def hash_func(self: Any) -> int:
        try:
            ...
            # and wrapping it in a `try` doesn't slow things down much in the
            # common case.
...
def complete_model_class(
    ...
    *,
    ...
    # of the properties are evaluated and the `ComputedFieldInfo` are
    # recreated:
    cls.__pydantic_computed_fields__ = {
