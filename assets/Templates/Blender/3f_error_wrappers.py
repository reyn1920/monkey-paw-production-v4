...
class ValidationError(Representation, ValueError):
    ...
    def __str__(self) -> str:
        ...
        return (
            f'{no_errors} validation error{
                "" if no_errors == 1 else "s"} for {
                self.model.__name__}\n'
            f"{
...
def display_errors(errors: List["ErrorDict"]) -> str:
    return "\n".join(
        f'{
            _display_error_loc(e)}\n  {
            e["msg"]} ({
                _display_error_type_and_ctx(e)})'
        for e in errors
    )
...
def get_exc_type(cls: Type[Exception]) -> str:
    # slightly more efficient than using lru_cache since we don't need to
    # worry about the cache filling up
    try:
