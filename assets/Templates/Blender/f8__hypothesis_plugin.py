...
try:
    ...
    def is_valid_email(s: str) -> bool:
        ...
        # that are invalid according to email-validator, so we filter those
        # out.
        try:
    ...
    # or other encoding issues; we're just trying to generate *something*
    # valid.
...
card_patterns = (
    # Note that these patterns omit the Luhn check digit; that's added by the
    # function above
    "4[0-9]{14}",  # Visa
...
# satisfying strategy.  First up, the machinery for tracking resolver
# functions:
...
def _registered(
    ...
    raise NotImplementedError(
        f"Unknown type {
            typ!r} has no resolver to register"
    )  # pragma: no cover
