...
class BaseSettings(BaseModel):
    ...
    def _build_values(
        ...
        # Provide a hook to set built-in sources priority and add / remove
        # sources
        ...
        def prepare_field(cls, field: ModelField) -> None:
            ...
            if env is None:
                ...
                raise TypeError(
                    f"invalid field env: {
                        env!r} ({
                        display_as_type(env)}); should be string, list or set"
                )
...
class EnvSettingsSource:
    ...
    def __call__(self, settings: BaseSettings) -> Dict[str, Any]:  # noqa C901
        ...
        for field in settings.__fields__.values():
            ...
            if is_complex:
                if env_val is None:
                    # field is complex but no value found so far, try
                    # explode_env_vars
                    env_val_built = self.explode_env_vars(field, env_vars)
                    if env_val_built:
                        d[field.alias] = env_val_built
                else:
                    # field is complex and there's a value, decode that as
                    # JSON, then add explode_env_vars
                    try:
    ...
    def explode_env_vars(
        ...
        for env_name, env_val in env_vars.items():
            if not any(env_name.startswith(prefix) for prefix in prefixes):
                continue
            # we remove the prefix before splitting in case the prefix has
    ...
    def __repr__(self) -> str:
        return (
            f"EnvSettingsSource(env_file={
                self.env_file!r}, env_file_encoding={
                self.env_file_encoding!r}, "
            f"env_nested_delimiter={
                self.env_nested_delimiter!r})"
        )
