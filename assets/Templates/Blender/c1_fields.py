...
class FieldInfo(Representation):
    ...
    # field constraints with the default value, it's also used in
    # update_from_config below
...
# used to be an enum but changed to int's for small performance
# improvement as less access overhead
SHAPE_SINGLETON = 1
SHAPE_LIST = 2
SHAPE_SET = 3
SHAPE_MAPPING = 4
...
class ModelField(Representation):
    ...
    def _get_field_info(
        ...
        if get_origin(annotation) is Annotated:
            ...
            if len(field_infos) > 1:
                raise ValueError(
                    f"cannot specify multiple `Annotated` `Field`s for {
                        field_name!r}"
                )
            ...
            if field_info is not None:
                ...
                if field_info.default not in (Undefined, Required):
                    raise ValueError(
                        f"`Field` default cannot be set in `Annotated` for {
                            field_name!r}"
                    )
        ...
        if isinstance(value, FieldInfo):
            if field_info is not None:
                raise ValueError(
                    f"cannot specify `Annotated` and value `Field`s together for {
                        field_name!r}"
                )
            field_info = value
            field_info.update_from_config(field_info_from_config)
    ...
    def _set_default_and_type(self) -> None:
        ...
        if self.default_factory is not None:
            if self.type_ is Undefined:
                raise errors_.ConfigError(
                    f"you need to set the type of field {
                        self.name!r} when using `default_factory`"
                )
            return
        ...
        if self.type_ is Undefined:
            raise errors_.ConfigError(
                f'unable to infer type for attribute "{
                    self.name}"'
            )
    ...
    def _type_analysis(self) -> None:  # noqa: C901 (ignore complexity)
        ...
        if self.type_ is Any or self.type_ is object:
            ...
            # python 3.7 only, Pattern is a typing object but without sub
            # fields
            return
        ...
        # add extra check for `collections.abc.Hashable` for python 3.10+ where
        # origin is not `None`
        if origin is None or origin is CollectionsHashable:
            ...
            if len(types_) == 1:
                # Optional[]
                self.type_ = types_[0]
                # this is the one case where the "outer type" isn't just the
                # original type
                self.outer_type_ = self.type_
        ...
        # type_ has been refined eg. as the type of a List and sub_fields needs
        # to be populated
    ...
    def _create_sub_type(
        ...
        if for_keys:
            ...
            # validators for sub items should not have `each_item` as we want
            # to check only the first sublevel
            class_validators = {
                k: Validator(
                    func=v.func,
                    pre=v.pre,
    ...
    def _validate_tuple(
        ...
        if not sequence_like(v):
            ...
            actual_length, expected_length = len(v), len(
                self.sub_fields
            )  # type: ignore
    ...
    def _get_mapping_value(
        ...
        if original_cls == dict or original_cls == Dict:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Could not convert dictionary to {
                        original_cls.__name__!r}"
                ) from None
    ...
    def _validate_singleton(
        ...
        if self.sub_fields:
            ...
            if self.model_config.smart_union and is_union(get_origin(self.type_)):
                ...
                # 2nd pass: check if the value is an instance of any subclass
                # of the Union types
                for field in self.sub_fields:
