...
# these are types that if empty, might be copied with simple copy()
# instead of deepcopy()
    ...
    def _normalize_indexes(
        ...
        for i, v in items.items():
            if not (
                ...
                raise TypeError(
                    f'Unexpected type of exclude value for index "{i}" {
                        v.__class__}'
                )
            if i == "__all__":
    ...
    if len(unique_aliases) > 1:
        raise ConfigError(
            f'Aliases for discriminator {
                discriminator_key!r} must be the same (got {
                ", ".join(
                    sorted(all_aliases))})'
        )
...
def get_discriminator_alias_and_values(
    ...
    if is_union(get_origin(tp)):
        ...
        if len(set(all_values)) > 1:
            raise ConfigError(
                f"Field {
                    discriminator_key!r} is not the same for all submodels of {
                    display_as_type(tp)!r}"
            )
        ...
        try:
            ...
            raise TypeError(
                f"Type {
                    tp.__name__!r} is not a valid `BaseModel` or `dataclass`"
            ) from e
        except KeyError as e:
            raise ConfigError(
                f"Model {
                    tp.__name__!r} needs a discriminator field for key {
                    discriminator_key!r}"
            ) from e
        ...
        if not is_literal_type(t_discriminator_type):
            raise ConfigError(
                f"Field {
                    discriminator_key!r} of model {
                    tp.__name__!r} needs to be a `Literal`"
            )
...
def _get_union_alias_and_all_values(
    ...
    # unzip: [('alias_a',('v1', 'v2)), ('alias_b', ('v3',))] => [('alias_a',
    # 'alias_b'), (('v1', 'v2'), ('v3',))]
