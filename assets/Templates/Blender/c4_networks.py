...
class UrlConstraints:
    ...
    def __get_pydantic_core_schema__(
        ...
        # that helps with validation on initialization, see _BaseUrl and
        # _BaseMultiHostUrl below.
        schema_to_mutate = (
...
class _BaseUrl:
    ...
    def serialize_url(cls, url: Any, info: core_schema.SerializationInfo) -> str | Self:
        if not isinstance(url, cls):
            raise PydanticSerializationUnexpectedValue(
                f"Expected `{cls}` but got `{
                    type(url)}` with value `'{url}'` - serialized value may not be as expected."
            )
        if info.mode == "json":
            return str(url)
        return url
    ...
    def __get_pydantic_json_schema__(
        ...
        # the function-wrap schema we use as a tool for validation on
        # initialization
        inner_schema = (
            core_schema["schema"]
...
# Where urls were annotated variants of `AnyUrl`, which was an alias to
# `pydantic_core.Url`
...
if TYPE_CHECKING:
    ...
    class IPvAnyNetwork:
        ...
        def __new__(cls, value: NetworkType) -> IPvAnyNetworkType:
            ...
            # Define your own class if you want to specify network address
            # check strictness.
            try:
