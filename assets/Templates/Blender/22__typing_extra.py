...
# TODO implement `is_finalvar_annotation` as Final can be wrapped with
# other special forms:
...
# Similarly, we shouldn't rely on this `_Final` class, which is even more
# private than `_GenericAlias`:
...
def parent_frame_namespace(
    *, parent_depth: int = 2, force: bool = False
    ...
    # modified down the line if this becomes a problem, we could implement
    # some sort of frozen mapping structure to enforce this.
    if force:
...
def eval_type_backport(
    ...
    try:
        ...
        message = f"Unable to evaluate type annotation {
            value.__forward_arg__!r}."
...
def _eval_type_backport(
    ...
    try:
        ...
        return eval_type_backport(
            ...
            globalns,
            localns,
            # pyright: ignore[reportArgumentType], waiting on a new
            # `eval_type_backport` release.
            try_default=False,
        )
...
def get_function_type_hints(
    ...
    *,
    ...
    if localns is None:
        ...
        # Pydantic has more advanced logic to do so (see
        # `_namespace_utils.ns_for_function`).
