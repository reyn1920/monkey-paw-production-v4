    ...
    def _pydantic_field_callback(self, ctx: FunctionContext) -> "Type":
        ...
        * Retrieve the type of the argument which is specified, and use it as return type for the function.
        ...
        if default_args:
            ...
            # Pydantic calls the default factory without any argument, so we
            ...
            if isinstance(default_factory_type, Overloaded):
                if MYPY_VERSION_TUPLE > (0, 910):
                    ...
                    # Mypy0.910 exposes the items of overloaded types in a
                    # function
            ...
            if isinstance(default_factory_type, CallableType):
                ...
                if args:
                    if all(isinstance(arg, TypeVarType) for arg in args):
                        # Looks like the default factory is a type like `list`
                        # or `dict`, replace all args with `Any`
                        ret_type.args = tuple(
...
class PydanticModelTransformer:
        ...
        if (
            ...
            for arg, name in zip(expr.args, expr.arg_names):
                ...
                if name == "default_factory":
                    return False
            # In this case, default and default_factory are not specified, so
            # we need to look at the annotation
            value_type = get_proper_type(cls.info[lhs.name].type)
            return not PydanticModelTransformer.type_has_implicit_default(value_type)
        # Only required if the "default value" is Ellipsis (i.e., `field_name:
        # Annotation = ...`)
    ...
    def type_has_implicit_default(type_: Optional[ProperType]) -> bool:
        ...
        if isinstance(type_, UnionType) and any(
            ...
            # Annotated as Optional, or otherwise having NoneType or AnyType in
            # the union
            return True
        return False
