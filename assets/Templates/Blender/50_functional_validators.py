...
class PlainValidator:
    ...
    def __get_pydantic_core_schema__(
        ...
        try:
            ...
            # schema validation will fail. That's why we use 'type ignore'
            # comments below.
...
def field_validator(
    ...
    *fields: str,
    ...
    if (
        ...
        raise PydanticUserError(
            f"`json_schema_input_type` can't be used when mode is set to {
                mode!r}",
...
if TYPE_CHECKING:
    # If we add configurable attributes to IsInstance, we'd probably need to
    # stop hiding it from type checkers like this
    ...
    class InstanceOf:
        ...
        def __get_pydantic_core_schema__(
            ...
            # use the generic _origin_ as the second argument to isinstance
            # when appropriate
            ...
            try:
                # Try to generate the "standard" schema, which will be used
                # when loading from JSON
                original_schema = handler(source)
            except PydanticSchemaGenerationError:
                # If that fails, just produce a schema that can validate from
                # python
