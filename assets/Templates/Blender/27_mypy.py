...
def from_attributes_callback(ctx: MethodContext) -> Type:
    ...
    if isinstance(ctx_type, CallableType) and isinstance(ctx_type.ret_type, Instance):
        ...
        detail = f"ctx.type: {ctx_type} (of type {
            ctx_type.__class__.__name__})"
...
class PydanticModelField:
    ...
    def expand_type(
        ...
        if force_typevars_invariant:
            ...
            # us problems in the future, we should look into why the
            # dataclasses plugin doesn't have this issue.
        ...
        if self.type is not None and self.info.self_type is not None:
            ...
            with state.strict_optional_set(api.options.strict_optional):
                ...
                # Cannot be TupleType as current_info represents a Pydantic
                # model:
                ...
                if (
                    ...
                    if root_type is None:
                        # Happens if the hint for 'root' has unsolved forward
                        # references
...
class PydanticModelTransformer:
        ...
        for sym in self._cls.info.names.values():
            if isinstance(sym.node, Decorator):
                ...
                if (
                    ...
                    # TODO: Only do this if the first argument of the decorated
                    # function is `cls`
    ...
    def collect_fields_and_class_vars(
        ...
        for info in reversed(
            ...
            # Each class depends on the set of attributes in its dataclass
            # ancestors.
            self._api.add_plugin_dependency(make_wildcard_trigger(info.fullname))
    ...
    def collect_field_or_class_var_from_stmt(  # noqa C901
        ...
        if not stmt.new_syntax:
            if (
                ...
                # Eventually, we may want to attempt to respect
                # model_config['ignored_types']
                return None

            if lhs.name in class_vars:
                # Class vars are not fields and are not required to be
                # annotated
                return None

            # The assignment does not have an annotation, and it's not anything
            # else we recognize
        ...
        if isinstance(node, PlaceholderNode):  # pragma: no cover
            ...
            # Basically, it is an edge case when dealing with complex import
            # logic

            # The dataclasses plugin now asserts this cannot happen, but I'd
            # rather not error if it does..
            return None
        ...
        if node.is_final and has_default:
            # TODO this path should be removed (see
        ...
        if setter:
            if isinstance(setter.node, FuncDef):
                ...
                if isinstance(setter_type, CallableType) and setter_type.arg_kinds == [
                    ...
                    self._api.fail(
                        f'Unsupported signature for "__set__" in "{
                            t.type.name}"',
                        context,
                    )
            else:
                self._api.fail(
                    f'Unsupported "__set__" in "{
                        t.type.name}"',
                    context,
                )
                ...
                if (
                    ...
                    for arg_idx, arg_name in enumerate(func_type.arg_names):
                        ...
                        if (
                            ...
                            # the Any type (as if CliSettingsSource was left
                            # unparameterized):
        ...
        for field in fields:
            ...
            if sym_node is not None:
                ...
                if isinstance(var, Var):
                    ...
                    # See https://github.com/pydantic/pydantic/issues/5191 to
                    # hit this branch for test coverage
                    self._api.defer()
                else:  # pragma: no cover
                    # I don't know whether it's possible to hit this branch,
                    # but I've added it for safety
                    try:
                        var_str = str(var)
                    except TypeError:
                        # This happens for PlaceholderNode; perhaps it will
                        # happen for other types in the future..
                        var_str = repr(var)
                    detail = f"sym_node.node: {var_str} (of type {
                        var.__class__})"
        ...
        if name == "extra":
            if isinstance(arg, StrExpr):
                ...
                if not lax_extra:
                    ...
                    # if you don't use an explicit string, the plugin won't be
                    # able to infer whether extra is forbidden.
    ...
    def get_has_default(stmt: AssignmentStmt) -> bool:
        ...
        if (
            ...
            return False
        # Has no default if the "default value" is Ellipsis (i.e., `field_name:
        # Annotation = ...`)
...
def error_unexpected_behavior(
    ...
    # Can't think of a good way to test this, but I confirmed it renders as
    # desired by adding to a non-error path
...
def add_method(
    ...
    # signature incompatible with the superclass, which causes mypy errors to
    # occur for every subclass of BaseModel.
    if is_classmethod:
