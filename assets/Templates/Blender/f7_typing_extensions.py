        ...
        if isinstance(cls, _ProtocolMeta) or sys.version_info >= (3, 12, 2):
            ...
            for attr in cls.__protocol_attrs__:
                try:
                    ...
                    raise TypeError(
                        f"Failed to determine whether protocol member {
                            attr!r} "
                        "is a method member"
                    ) from e
                else:
...
if _NEEDS_SINGLETONMETA:
    ...
    class SingletonMeta(type):
        def __setattr__(cls, attr, value):
            ...
            raise TypeError(
                f"cannot set {
                    attr!r} attribute of immutable type {
                    cls.__name__!r}"
            )
    ...
    class _TypedDictMeta(type):
        ...
        def __new__(
            ...
            if closed is not None and extra_items is not NoExtraItems:
                raise TypeError(
                    f"Cannot combine closed={
                        closed!r} and extra_items"
                )
...
if hasattr(typing, "ReadOnly"):  # 3.13+
    ...
    def _clean_optional(obj, hints, globalns=None, localns=None):
        ...
        for name, value in hints.items():
            ...
            # value=NoneType should have caused a skip above but check for
            # safety
            ...
            # value might be cached by typing._tp_cache contrary to
            # original_evaluated
            if original_evaluated != value or (
                # 3.10: ForwardRefs of UnionType might be turned into
                # _UnionGenericAlias
...
# 3.10+
    ...
    if sys.version_info < (3, 11):
        ...
        class _ConcatenateGenericAlias(typing._ConcatenateGenericAlias, _root=True):
            # needed for checks in collections.abc.Callable to accept this
            # class
            __module__ = "typing"
            ...
            def _typevartuple_prepare_subst(alias, args):
                ...
                if var_tuple_index is not None:
                    ...
                    raise TypeError(
                        ...
                        f" actual {alen}, expected at least {
                            plen - 1}"
                    )
                ...
                if key in _prohibited_namedtuple_fields:
                    ...
                    try:
                        ...
                        try:
                            ...
                            msg = (
                                f"Error calling __set_name__ on {
                                    type(val).__name__!r} "
                                f"instance {
                                    key!r} in {
                                    typename!r}"
                            ...
                            # Breakpoint:
                            # https://github.com/python/cpython/pull/95915
                            if sys.version_info >= (3, 12):
                                e.add_note(msg)
                                raise
                            else:
...
# <=3.13
    ...
    class TypeAliasType:
        ...
        def __getitem__(self, parameters):
            ...
            # as it is converted to a list from which no parameters are
            # extracted.
