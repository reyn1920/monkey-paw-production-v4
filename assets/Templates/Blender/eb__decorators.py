...
class DecoratorInfos:
    ...
    def build(model_dc: type[Any]) -> DecoratorInfos:  # noqa: C901 (ignore complexity)
        ...
        for var_name, var_value in vars(model_dc).items():
            if isinstance(var_value, PydanticDescriptorProxy):
                ...
                if isinstance(info, ValidatorDecoratorInfo):
                    ...
                    # check whether a serializer function is already registered
                    # for fields
        ...
        if to_replace:
            ...
            # which allows validator class methods to also function as regular
            # class methods
...
def inspect_validator(validator: Callable[..., Any], mode: FieldValidatorModes) -> bool:
    ...
    if mode == "wrap":
        ...
        assert mode in {
            ...
            mode!r}, expected 'before', 'after' or 'plain"
        if n_positional == 2:
            return True
        elif n_positional == 1:
            return False
...
def inspect_field_serializer(
    ...
    try:
        ...
        # `inspect.signature` might not be able to infer a signature, e.g. with C objects.
        # In this case, we assume no info argument is present and this is not a
        # method:
        return (False, False)
    ...
    *,
    ...
    while isinstance(func, unwrap_types):
        if unwrap_class_static_method and isinstance(func, (classmethod, staticmethod)):
            ...
            # Make coverage happy as it can only get here in the last possible
            # case
