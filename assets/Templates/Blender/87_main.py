...
# If a field is of type `Callable`, its default value should be a function
# and cannot to ignored.
...
# When creating a `BaseModel` instance, we bypass all the methods,
# properties... added to the model
...
class ModelMetaclass(ABCMeta):
    ...
    def __new__(mcs, name, bases, namespace, **kwargs):  # noqa C901
        ...
        # set __signature__ attr only for model class, but not for its
        # instances
        ...
        if not _is_base_model_class_defined:
            ...
            # BaseModel's body (where annotations are set), so clear them
            # manually:
...
class BaseModel(Representation, metaclass=ModelMetaclass):
    ...
    def json(
        ...
        **dumps_kwargs: Any,
        ...
        # This allows users to write custom JSON encoders for given `BaseModel`
        # classes.
        data = dict(
            self._iter(
    ...
    def _copy_and_set_values(
        ...
        if deep:
            # chances of having empty dict here are quite low for using
            # smart_deepcopy
    ...
    def copy(
        ...
        *,
        ...
        # new `__fields_set__` can have unset optional fields with a set value
        # in `update` kwarg
        if update:
    ...
    def validate(cls: Type["Model"], value: Any) -> "Model":
        if isinstance(value, cls):
            ...
            # whether to deep or shallow copy the model on validation, None
            # means do not copy
    ...
    def _iter(
        ...
        # The extra "is not None" guards are not logically necessary but
        # optimizes performance for the simple case.
