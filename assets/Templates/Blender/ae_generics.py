...
class GenericModel(BaseModel):
    ...
    # Setting the return type as Type[Any] instead of Type[BaseModel] prevents
    # PyCharm warnings
    def __class_getitem__(
        ...
        if not hasattr(cls, "__parameters__"):
            raise TypeError(
                f"Type {
                    cls.__name__} must inherit from typing.Generic before being parameterized"
            )
        ...
        created_model = cast(
            # casting ensures mypy is aware of the __concrete__ and
            # __parameters__ attributes
            Type[GenericModel],
            create_model(
                model_name,
    ...
    def __parameterized_bases__(
        ...
        for base_model in cls.__bases__:
            if not issubclass(base_model, GenericModel):
                ...
                # or
                # base_model is already concrete, and will be included
                # transitively via cls.
                continue
                ...
                if base_model in _assigned_parameters:
                    ...
                    # Need to determine the mapping for the base_model
                    # parameters
                ...
                # can construct the Parameterised base model using typevars_map
                # directly
...
def check_parameters_count(
    ...
    if actual != expected:
        ...
        raise TypeError(
            f"Too {description} parameters for {
                cls.__name__}; actual {actual}, expected {expected}"
        )
