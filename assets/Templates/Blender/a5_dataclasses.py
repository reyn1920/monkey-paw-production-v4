    ...
    def make_pydantic_fields_compatible(cls: type[Any]) -> None:
        ...
        for annotation_cls in cls.__mro__:
            ...
            for field_name in annotations:
                ...
                # Set `repr` attribute if it's explicitly specified to be not
                # `True`.
                ...
                # for annotations, so we must make sure it's initialized before
                # we add to it.
    ...
    def create_dataclass(cls: type[Any]) -> type[PydanticDataclass]:
        ...
        if is_model_class(cls):
            raise PydanticUserError(
                f"Cannot create a Pydantic dataclass from {
                    cls.__name__} as it is already a Pydantic model",
        ...
        # because a dataclass base might provide a __pydantic_config__
        # attribute that we don't want to warn about
        ...
        if (
            ...
            warn(
                f"`config` is set via both the `dataclass` decorator and `__pydantic_config__` for dataclass {
                    cls.__name__}. "
        ...
        # Respect frozen setting from dataclass constructor and fallback to
        # config setting if not provided
        if frozen is not None:
            frozen_ = frozen
            if config_wrapper.frozen:
                # It's not recommended to define both, as the setting from the
                # dataclass decorator will take priority.
                warn(
        ...
        cls = dataclasses.dataclass(  # type: ignore[call-overload]
            cls,
            # the value of init here doesn't affect anything except that it
            # makes it easier to generate a signature
            init=True,
            repr=repr,
            eq=eq,
            order=order,
        ...
        # and possibly cache it (see the `__pydantic_parent_namespace__` logic
        # for models).
...
if sys.version_info < (3, 11):
    ...
    # Starting in 3.11, typing.get_type_hints will not raise an error if the
    # retrieved type hints are not callable.
    ...
    *,
    ...
    for attr in (
        ...
        if attr in cls.__dict__:
            ...
            # pydantic-core. Same applies for the core schema that can be
