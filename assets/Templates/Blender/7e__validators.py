...
def sequence_validator(
    ...
    if value_type is list:
        ...
        # best guess at how to re-create the original type, more custom
        # construction logic might be required
...
def import_string(value: Any) -> Any:
    if isinstance(value, str):
        ...
        # otherwise we just return the value and let the next validator do the
        # rest of the work
        return value
...
def _import_string_logic(dotted_path: str) -> Any:
    ...
    * the substring of `dotted_path` after the colon is not an attribute of the module (e.g., 'collections:abc123')
    ...
    if len(components) > 2:
        raise ImportError(
            f"Import strings should have at most one ':'; received {
                dotted_path!r}"
        )
    ...
    if not module_path:
        raise ImportError(
            f"Import strings should have a nonempty module name; received {
                dotted_path!r}"
        )
    ...
    try:
        ...
        if "." in module_path:
            # Check if it would be valid if the final item was separated from
            # its module with a `:`
...
def _extract_decimal_digits_info(decimal: Decimal) -> tuple[int, int]:
    ...
    try:
        ...
        if exponent >= 0:
            ...
            # Ex: digit_tuple=(1, 2, 3), exponent=2 -> 12300 -> 0 decimal
            # places, 5 digits
            num_digits += exponent
            decimal_places = 0
        else:
            ...
            # Ex: digit_tuple=(1, 2, 3), exponent=-4 -> 0.0123 -> 4 decimal
            # places, 4 digits
...
def get_defaultdict_default_default_factory(
    ...
    def infer_default() -> Callable[[], Any]:
        ...
        if typing_objects.is_typevar(values_type):
            ...
            # a somewhat subjective set of types that have reasonable default
            # values
            allowed_msg = ", ".join(
