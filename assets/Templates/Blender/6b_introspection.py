...
def get_literal_values(
    ...
    *,
    ...
    if unpack_type_aliases == "skip":
        ...
        # We'll need to manually deduplicate parameters, see the `Literal`
    ...
    while True:
        ...
        if origin is not None:
            if typing_objects.is_classvar(origin):
                ...
                # origin is not None but not a type qualifier nor `Annotated`
                # (e.g. `list[int]`):
                break
    ...
    if check_annotated and typing_objects.is_annotated(origin):
        ...
        try:
            ...
            if metadata:
                ...
                # for this, as we don't want to unpack other type aliases (e.g.
                # `type MyInt = int`).
                return typ, metadata
            return annotation, []
        ...
        try:
            ...
            try:
                ...
                # we do `Annotated[list[T], ...][int]` (which gives
                # `Annotated[list[T], ...]`):
