...
class _ApplyInferredDiscriminator:
    ...
    * Handling various edge cases such as 'definitions', 'default', 'nullable' schemas, and more
    ...
    def __init__(
        ...
        # This may change if/when we expose a way to manually specify the
        # TaggedUnionSchema's choices.
        ...
        # more complex cases, without over-complicating the inference logic for
        # the common cases.
        ...
        # python side, but resolves the issue that `None` cannot correspond to
        # any discriminator values.
        ...
        # to the possible presence of other wrapper schemas such as
        # DefinitionsSchema, WithDefaultSchema, etc.
        ...
        # algorithm may add more choices to this stack as (nested) unions are
        # encountered.
        ...
        # in the output TaggedUnionSchema that will replace the union from the
        # input schema
    ...
    def _apply_to_root(self, schema: core_schema.CoreSchema) -> core_schema.CoreSchema:
        ...
        if schema["type"] != "union":
            ...
            # as a way to get discriminated-union-style error messages, so we
            # allow this here.
            schema = core_schema.union_schema([schema])

        # Reverse the choices list before extending the stack so that they get
        # handled in the order they occur
        choices_schemas = [
        ...
        if (
            ...
            # is the appropriate way to provide a list of fallback attributes
            # to check for a discriminator value.)
    ...
    def _handle_choice(self, choice: core_schema.CoreSchema) -> None:
        ...
        * Updating the _tagged_union_choices mapping that will ultimately be used to build the TaggedUnionSchema.
        ...
        if choice["type"] == "none":
            ...
            # Reverse the choices list before extending the stack so that they
            # get handled in the order they occur
            choices_schemas = [
            ...
            err_str = f'The core schema type {
                choice["type"]!r} is not a valid discriminated union variant.'
            if choice["type"] == "list":
                err_str += (
            ...
            if choice["type"] == "tagged-union" and self._is_discriminator_shared(
                ...
                # and its choices can be coalesced into the outer
                # TaggedUnionSchema.
                subchoices = [
                    x
                ...
                # Reverse the choices list before extending the stack so that
                # they get handled in the order they occur
    ...
    def _infer_discriminator_values_for_choice(  # noqa C901
        ...
        if choice["type"] == "definitions":
            ...
            values: list[str | int] = []
            # Ignore str/int "choices" since these are just references to other
            # choices
            subchoices = [
    ...
    def _infer_discriminator_values_for_typed_dict_choice(
        ...
        if field is None:
            raise PydanticUserError(
                f"{source} needs a discriminator field for key {
                    self.discriminator!r}",
    ...
    def _infer_discriminator_values_for_field(
        ...
        if field["type"] == "computed-field":
            # This should never occur as a discriminator, as it is only
        ...
        if not isinstance(alias, str):
            raise PydanticUserError(
                f"Alias {
                    alias!r} is not supported in a discriminated union",
        ...
        if self._discriminator_alias is None:
            ...
            raise PydanticUserError(
                f"Aliases for discriminator {
                    self.discriminator!r} must be the same "
                f"(got {alias}, {
                    self._discriminator_alias})",
    ...
    def _infer_discriminator_values_for_inner_schema(
        ...
        if schema["type"] == "literal":
            ...
            # For example, this lets us handle `Union[Literal['key'],
            # Union[Literal['Key'], Literal['KEY']]]`
            values: list[Any] = []
            ...
            # This will happen if the field has a default value; we ignore it
            # while extracting the discriminator values
            return self._infer_discriminator_values_for_inner_schema(
            ...
            raise PydanticUserError(
                f"{source} needs field {
                    self.discriminator!r} to be of type `Literal`",
    ...
    def _set_unique_choice_for_values(
        ...
        for discriminator_value in values:
            if discriminator_value in self._tagged_union_choices:
                ...
                # until we get to a "real" choice, and confirm that is equal to
                # the one assigned.
