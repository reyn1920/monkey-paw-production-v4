...
class WebSocket(HTTPConnection):
    ...
    async def receive(self) -> Message:
        ...
        if self.client_state == WebSocketState.CONNECTING:
            ...
            if message_type != "websocket.connect":
                raise RuntimeError(
                    f'Expected ASGI message "websocket.connect", but got {
                        message_type!r}'
                )
            ...
            if message_type not in {"websocket.receive", "websocket.disconnect"}:
                raise RuntimeError(
                    f'Expected ASGI message "websocket.receive" or "websocket.disconnect", but got {
                        message_type!r}'
                )
    ...
    async def send(self, message: Message) -> None:
        ...
        if self.application_state == WebSocketState.CONNECTING:
            ...
            if message_type not in {"websocket.send", "websocket.close"}:
                raise RuntimeError(
                    f'Expected ASGI message "websocket.send" or "websocket.close", but got {
                        message_type!r}'
                )
            if message_type == "websocket.close":
                self.application_state = WebSocketState.DISCONNECTED
            ...
            if message_type != "websocket.http.response.body":
                raise RuntimeError(
                    f'Expected ASGI message "websocket.http.response.body", but got {
                        message_type!r}'
                )
    ...
    async def accept(
        ...
        if self.client_state == WebSocketState.CONNECTING:  # pragma: no branch
            # If we haven't yet seen the 'connect' message, then wait for it
            # first.
            await self.receive()
        await self.send(
