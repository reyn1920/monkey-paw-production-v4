...
# specific recursive generic models without losing required items out of
# the cache.
...
def replace_types(type_: Any, type_map: Mapping[TypeVar, Any] | None) -> Any:
    ...
    if type_args:
        ...
        # NotRequired[T] and Required[T] don't support tuple type
        # resolved_type_args, hence the condition below
        return origin_type[
            (
...
def map_generic_model_arguments(
    ...
    for parameter, argument in zip_longest(parameters, args, fillvalue=_missing):
        if parameter is _missing:
            raise TypeError(
                f"Too many arguments for {cls}; actual {
                    len(args)}, expected {expected_len}"
            )
        ...
        if argument is _missing:
            ...
            try:
                ...
                # Happens if using `typing.TypeVar` (and not
                # `typing_extensions`) on Python < 3.13.
                has_default = False
            if has_default:
                ...
                raise TypeError(
                    f"Too few arguments for {cls}; actual {
                        len(args)}, expected at least {expected_len}"
                )
        else:
            param = typing.cast(TypeVar, parameter)
...
def _late_cache_key(
    ...
    # whereas this function will always produce a tuple as the first item in
    # the key.
