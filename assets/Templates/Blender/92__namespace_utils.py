...
class NsResolver:
    ...
    def types_namespace(self) -> NamespacesTuple:
        ...
        if not self._types_stack:
            ...
            # return NamespacesTuple(self._base_ns_tuple.globals,
        ...
        locals_list: list[MappingNamespace] = []
        # Hacky workarounds, see class docstring:
        # An optional parent namespace that will be added to the locals with
        # the lowest priority
        if self._parent_ns is not None:
        ...
        if type_params:
            ...
            # value, e.g. `type A[T] = 'T'` (which is not necessary in most
            # cases).
            locals_list.append({t.__name__: t for t in type_params})
        ...
        # The `len(self._types_stack) > 1` check above prevents this from being
        # added twice:
