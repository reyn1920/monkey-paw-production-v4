...
class CoreConfig(TypedDict, total=False):
    ...
    loc_by_alias: bool
    # whether instances of models and dataclasses (including subclass
    # instances) should re-validate, default 'never'
...
def plain_serializer_function_ser_schema(
    ...
    *,
    ...
    """
    if when_used == "always":
        # just to avoid extra elements in schema, and to use the actual default
        # defined in rust
...
class DateSchema(TypedDict, total=False):
    ...
    # value is restricted to -86_400 < offset < 86_400 by bounds in
...
class UnionSchema(TypedDict, total=False):
    ...
    # default true, whether to automatically collapse unions with one element
...
class TypedDictSchema(TypedDict, total=False):
    ...
    # all these values can be set via config, equivalent fields have
    # `typed_dict_` prefix
...
# union which kills performance not just for pydantic, but even for code
# using pydantic
if not MYPY:
    CoreSchema = Union[
        InvalidSchema,
...
# to update this, call `pytest -k test_core_schema_type_literal` and copy
# the output
...
def __getattr__(attr_name: str) -> object:
    ...
    if new_attr is None:
        ...
        msg = f"`{attr_name}` is deprecated, use `{
            new_attr.__name__}` instead."
