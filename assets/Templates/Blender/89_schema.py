...
def get_field_schema_validations(field: ModelField) -> Dict[str, Any]:
    ...
    if lenient_issubclass(field.type_, Enum):
        # schema is already updated by `enum_process_schema`; just update with
        # field extra
...
def field_type_schema(
    ...
    *,
    ...
    # check field type to avoid repeated calls to the same __modify_schema__
    # method
    ...
    if len(sub_fields) == 1:
        ...
        if field_has_discriminator:
            ...
            for discriminator_value, sub_field in field.sub_fields_mapping.items():
                ...
                # sub_field is either a `BaseModel` or directly an `Annotated`
                # `Union` of many
        ...
        for sf in sub_fields:
            ...
            if sub_schema.keys() == {"discriminator", "oneOf"}:
                # we don't want discriminator information inside oneOf choices,
...
# this is used only for standard library types, custom types should use
# __modify_schema__ instead
...
def add_field_type_to_schema(field_type: Any, schema_: Dict[str, Any]) -> None:
    ...
    for type_, t_schema in field_class_to_schema:
        # Fallback for `typing.Pattern` and `re.Pattern` as they are not a
        # valid class
...
def field_singleton_schema(  # noqa: C901 (ignore complexity)
    ...
    *,
    ...
    if is_callable_type(field_type):
        raise SkipField(
            f"Callable {
                field.name} was excluded from schema since JSON schema has no equivalent type."
        )
    f_schema: Dict[str, Any] = {}
    if field.field_info is not None and field.field_info.const:
        f_schema["const"] = field.default
