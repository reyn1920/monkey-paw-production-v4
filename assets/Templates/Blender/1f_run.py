...
class CombinedProcess:
    ...
    def stop(self, sigint_timeout: int = 5, sigkill_timeout: int = 1) -> None:
        ...
        if self.is_alive():
            ...
            try:
                ...
                # This will allow the SIGKILL to be sent, otherwise it is
                # swallowed up.
    ...
    @property
    def pid(self) -> int:
        # we check the process has always been spawned when CombinedProcess is
        # initialised
...
def set_tty(tty_path: Optional[str]) -> Generator[None, None, None]:
    if tty_path:
        try:
            ...
            # eg. "No such device or address: '/dev/tty'", see
            # https://github.com/samuelcolvin/watchfiles/issues/40
            yield
    else:
        # currently on windows tty_path is None and there's nothing we can do
        # here
        yield
