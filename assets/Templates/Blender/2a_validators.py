...
def str_validator(v: Any) -> Union[str]:
    if isinstance(v, str):
        ...
        # is there anything else we want to add here? If you think so, create
        # an issue.
        return str(v)
...
# matches the default limit cpython, see
...
def int_validator(v: Any) -> int:
    ...
    # 1. update to the latest patch release of python once released, 2. use a
    # different JSON library like ujson
...
def make_literal_validator(type_: Any) -> Callable[[Any], Any]:
    ...
    # In some cases the set value and checked value can indeed be different
    # (see `test_literal_validator_str_enum`)
...
def find_validators(  # noqa: C901 (ignore complexity)
    ...
    for val_type, validators in _VALIDATORS:
        try:
            ...
            raise RuntimeError(
                f"error checking inheritance of {
                    type_!r} (type: {
                    display_as_type(type_)})"
            )
    ...
    if config.arbitrary_types_allowed:
        ...
        if hasattr(type_, "__pydantic_core_schema__"):
            warn(
                f"Mixing V1 and V2 models is not supported. `{
                    type_.__name__}` is a V2 model.",
                UserWarning,
            )
        raise RuntimeError(
