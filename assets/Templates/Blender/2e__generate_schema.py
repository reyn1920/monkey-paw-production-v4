...
def _add_custom_serialization_from_json_encoders(
    ...
    # if the type is a GenericAlias (e.g. from list[int]) we need to use
    # __class__ instead of .__mro__
    for base in (tp, *getattr(tp, "__mro__", tp.__class__.__mro__)[:-1]):
        ...
        warnings.warn(
            f"`json_encoders` is deprecated. See https://docs.pydantic.dev/{
                version_short()}/concepts/serialization/#custom-serializers for alternatives",
            PydanticDeprecatedSince20,
        )

        # TODO: in theory we should check that the schema accepts a
        # serialization key
...
class GenerateSchema:
    ...
    def _enum_schema(self, enum_type: type[Enum]) -> CoreSchema:
        ...
        # This is the default value provided by enum.EnumMeta.__new__; don't
        # use it
        ...
        if issubclass(enum_type, int):
            ...
            # TODO this is an ugly hack, how do we trigger an Any schema for
            # serialization?
        ...
        if cases:
            ...
            # we don't want to add the missing to the schema if it's the
            # default one
            default_missing = (
        ...
        def ser_ip(ip: Any, info: core_schema.SerializationInfo) -> str | IpType:
            if not isinstance(ip, (tp, str)):
                raise PydanticSerializationUnexpectedValue(
                    f"Expected `{tp}` but got `{
                        type(ip)}` with value `'{ip}'` - serialized value may not be as expected."
                )
            if info.mode == "python":
                return ip
            return str(ip)
    ...
    def _path_schema(self, tp: Any, path_type: Any) -> CoreSchema:
        ...
        def ser_path(
            ...
            if not isinstance(path, (tp, str)):
                raise PydanticSerializationUnexpectedValue(
                    f"Expected `{tp}` but got `{
                        type(path)}` with value `'{path}'` - serialized value may not be as expected."
                )
            if info.mode == "python":
                return path
            return str(path)
    ...
    def _deque_schema(self, items_type: Any) -> CoreSchema:
        ...
        # items via a list schema, but it's ok if the deque itself is not a
        # list
    ...
    def _add_js_function(
        ...
        # or something like that, but if it does it'll fail by inserting the
        # duplicate
            ...
            with (
                ...
                if cls.__pydantic_fields_complete__ or cls is BaseModel_:
                    ...
                    if not hasattr(cls, "__pydantic_fields__"):
                        ...
                        # a schema for `t: Other`. As
                        # `Other.__pydantic_fields__` aren't set, we abort.
    ...
    def _generate_schema_from_get_schema_method(
        ...
        is_base_model_get_schema = (
            ...
            is BaseModel_.__get_pydantic_core_schema__.__func__
        )
        ...
        if (
            ...
            # safety measure (because these are inlined in place -- i.e.
            # mutated directly)
            return schema
        ...
        if (
            ...
            if issubclass(obj, BaseModelV1):
                warn(
                    f"Mixing V1 models and V2 models (or constructs, like `TypeAdapter`) is not supported. Please upgrade `{
                        obj.__name__}` to V2.",
                    UserWarning,
                )
            else:
    ...
    def _resolve_forward_ref(self, obj: Any) -> Any:
        ...
        # if obj is still a ForwardRef, it means we can't evaluate it, raise
        # PydanticUndefinedAnnotation
    ...
    def _get_args_resolving_forward_refs(
        ...
        if args:
            if isinstance(obj, GenericAlias):
                # PEP 585 generic aliases don't convert args to ForwardRefs,
                # unlike `typing.List/Dict` etc.
                args = (
    ...
    def _match_generic_type(self, obj: Any, origin: Any) -> CoreSchema:  # noqa: C901
        ...
        # to resolve by modifying the value returned by
        # `Generic.__class_getitem__`, but that is a dangerous game.
        ...
        if (
            ...
            if isinstance(alias_generator, AliasGenerator):
                ...
                if not isinstance(alias, str):
                    raise TypeError(
                        f"alias_generator {alias_generator} must return str, not {
                            alias.__class__}"
                    )
            ...
            # if the priority is 1, then we set the aliases to the generated
            # alias
            ...
            # if any of the aliases are not set, then we set them to the
            # corresponding generated alias
    ...
    def _apply_field_title_generator_to_field_info(
        ...
        if field_info.title is None:
            ...
            if not isinstance(title, str):
                raise TypeError(
                    f"field_title_generator {field_title_generator} must return str, not {
                        title.__class__}"
                )
    ...
    def _common_field_schema(  # C901
        ...
        # Convert `@field_validator` decorators to
        # `Before/After/Plain/WrapValidator` instances:
    ...
    def _namedtuple_schema(
        ...
        with (
            ...
            try:
                ...
                raise PydanticUndefinedAnnotation.from_name_error(e) from e
            if not annotations:
                # annotations is empty, happens if namedtuple_cls defined via
    ...
    def _tuple_schema(self, tuple_type: Any) -> core_schema.CoreSchema:
        ...
        # This is only true for <3.11, on Python 3.11+ `typing.Tuple[()]` gives
        # `params=()`
        if not params:
            if tuple_type in TUPLE_TYPES:
                ...
                # special case for `tuple[()]` which means `tuple[]` - an empty
                # tuple
                return core_schema.tuple_schema([])
            ...
            # NOTE: This conditional can be removed when we drop support for
            # Python 3.10.
            return core_schema.tuple_schema([])
        else:
            return core_schema.tuple_schema(
        ...
        if typing_objects.is_any(type_param):
            ...
            if _typing_extra.is_generic_alias(type_param):
                raise PydanticUserError(
                    ...
                    f"Instead of using type[{
                        type_param!r}], use type[{
                        _repr.display_as_type(
                            get_origin(type_param))}].",
                    code=None,
                )
        ...
        with (
            ...
            with (
                ...
                if self._config_wrapper.extra == "allow":
                    # disallow combination of init=False on a dataclass field
                    # and extra='allow' on a dataclass
                ...
                # Note that when kw_only is missing or None, it is treated as
                # equivalent to kw_only=True
                args = sorted(
                    (
            ...
            if parameter_mode is not None:
                ...
                if unpack_type is not None:
                    ...
                    if not is_typeddict(origin):
                        raise PydanticUserError(
                            f"Expected a `TypedDict` class inside `Unpack[...]`, got {
                                unpack_type!r}",
                    ...
                    if overlapping_params:
                        raise PydanticUserError(
                            f"Typed dictionary {
                                origin.__name__!r} overlaps with parameter"
                            f'{
                                "s" if len(overlapping_params) >= 2 else ""} '
                            f'{
                                ", ".join(
                                    repr(p) for p in sorted(overlapping_params))}',
    ...
    def _unsubstituted_typevar_schema(
        ...
        try:
            ...
            # Happens if using `typing.TypeVar` (and not `typing_extensions`)
            # on Python < 3.13
            pass
        else:
            if has_default:
    ...
    def _apply_single_annotation(
        ...
        if schema["type"] == "nullable":
            # for nullable schemas, metadata is automatically applied to the
            # inner schema
    ...
    def _apply_field_serializers(
        ...
        if serializers:
            ...
            # use the last serializer to make it easy to override a serializer
            # set on a parent model
...
class _Definitions:
    ...
    def get_schema_or_ref(
        ...
        """
        ref = get_type_ref(tp)
        # return the reference if we're either (1) in a cycle or (2) it the
        # reference was already encountered:
    ...
    def finalize_schema(self, schema: CoreSchema) -> CoreSchema:
        ...
        # and references are encountered. See the
        # `test_deferred_discriminated_union_and_references()` test.
        for ref, inlinable_def_ref in gather_result["collected_references"].items():
            if (
                ...
                if inlining_behavior == "inline":
                    ...
                    # sure to keep the metadata for the deferred discriminator
        ...
        for cs in gather_result["deferred_discriminator_schemas"]:
            ...
            if discriminator is None:
                ...
                # In this case, a previous loop iteration applied the
                # discriminator and so we can just skip it here.
                continue
    ...
    def _resolve_definition(
        ...
        # a PEP 695 type alias (which is referenceable) that references another
        # PEP 695 type alias):
        visited: set[str] = set()
        while (
