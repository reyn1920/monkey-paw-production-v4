...
def apply_known_metadata(
    ...
    for constraint, value in schema_update.items():
        ...
        # whereas other times we want to wrap the existing schema with a new
        # one that enforces a new constraint.
        if (
        ...
        # if we're allowed to apply constraint directly to the schema, like le
        # to int, do that
        ...
        # else, apply a function after validator to the schema to enforce the
        ...
        if constraint in chain_schema_constraints:
            ...
            # Most constraint errors are caught at runtime during attempted
            # application
            raise RuntimeError(
    ...
    for annotation in other_metadata:
        if (annotation_type := type(annotation)) in (
            ...
            def val_func(v: Any) -> Any:
                ...
                # annotation.func may also raise an exception, let it pass
                # through
...
def collect_known_metadata(
    ...
    for annotation in annotations:
        # isinstance(annotation, PydanticMetadata) also covers
        # ._fields:_PydanticGeneralMetadata
        if isinstance(annotation, PydanticMetadata):
            res.update(annotation.__dict__)
        # we don't use dataclasses.asdict because that recursively calls asdict
        # on the field values
            ...
            # e.g. `Annotated[int, Strict]` as well as `Annotated[int,
            # Strict()]`
            res.update(
