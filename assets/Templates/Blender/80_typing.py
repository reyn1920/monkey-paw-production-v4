...
try:
    ...
    # python < 3.9 does not have GenericAlias (list[int], tuple[str, ...] and
    # so on)
    TypingGenericAlias = ()
...
if sys.version_info < (3, 8):
    ...
    def get_args(tp: Type[Any]) -> Tuple[Any, ...]:
        ...
        # the fallback is needed for the same reasons as `get_origin` (see
        # above)
        return (
...
if sys.version_info < (3, 9):
    ...
    def convert_generics(tp: Type[Any]) -> Type[Any]:
        ...
        # recursively replace `str` instances inside of `GenericAlias` with
        # `ForwardRef(arg)`
        converted = tuple(
            (
                ForwardRef(arg)
