    ...
    **Note:** `TypeAdapter` instances are not types, and cannot be used as type annotations for fields.
    ...
    def __init__(
        ...
        *,
        ...
        if parent_frame is not None:
            ...
            # Do not provide a local ns if the type adapter happens to be
    ...
    def _fetch_parent_frame(self) -> FrameType | None:
        ...
        if frame.f_globals.get("__name__") == "typing":
            ...
            # pulling anything from the `typing` module, use the correct frame
            # (the one before):
            return frame.f_back
    ...
    def _init_core_attrs(
        ...
        try:
            ...
            # so we simply skip to the block below that does the actual schema
            # generation
            if (
    ...
    def rebuild(
        ...
        *,
        ...
        # and so we skip the globalns = get_module_ns_of(typ) call that would
        # normally happen
    ...
    def json_schemas(
        ...
        *,
        ...
        for key, mode, adapter in inputs:
            # This is the same pattern we follow for model json schemas - we
            # attempt a core schema rebuild if we detect a mock
