...
def has_operator(
    typ: Type, op_method: str, named_type: Callable[[str], Instance]
) -> bool:
    ...
    * instance level attributes (i.e. extra_attrs) are not considered
    ...
    if isinstance(typ, TypeType):
        ...
        if isinstance(item, UnionType):
            return all(
                meta_has_operator(x, op_method, named_type)
                for x in item.relevant_items()
            )
...
def instance_fallback(
    typ: ProperType, named_type: Callable[[str], Instance]
) -> Instance:
    if isinstance(typ, Instance):
        return typ
...
def meta_has_operator(
    item: Type, op_method: str, named_type: Callable[[str], Instance]
) -> bool:
