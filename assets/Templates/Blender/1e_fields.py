...
class FieldInfo(_repr.Representation):
    ...
    # None has a special meaning - these items are collected into a
    ...
    def __init__(self, **kwargs: Unpack[_FieldInfoInputs]) -> None:
        ...
        # For compatibility with FastAPI<=0.110.0, we preserve the existing
        # value if it is not overridden
    ...
    def merge_field_infos(*field_infos: FieldInfo, **overrides: Any) -> FieldInfo:
        ...
        if len(field_infos) == 1:
            # No merging necessary, but we still need to make a copy and apply
            # the overrides
        ...
        for field_info in field_infos:
            ...
            try:
                ...
                if isinstance(existing_json_schema_extra, dict):
                    ...
                    # if ever there's a case of a callable, we'll just keep the
                    # last json schema extra spec
                    merged_field_info_kwargs["json_schema_extra"] = json_schema_extra
            except KeyError:
                pass

            # later FieldInfo instances override everything except
            # json_schema_extra from earlier FieldInfo instances
    ...
    def _from_dataclass_field(dc_field: DataclassField[Any]) -> FieldInfo:
        ...
        # use the `Field` function so in correct kwargs raise the correct
        # `TypeError`
        dc_field_metadata = {
...
# type hint the return value as `Any` to avoid type checking regressions
# when using `...`.
@overload
def Field(
...
def Field(  # noqa: C901
    ...
    **extra: Unpack[_EmptyKwargs],
    ...
    """
    # Check deprecated and removed params from V1. This logic should
...
class ModelPrivateAttr(_repr.Representation):
    ...
    if not typing.TYPE_CHECKING:
        # We put `__getattr__` in a non-TYPE_CHECKING block because otherwise,
        # mypy allows arbitrary attribute access
...
def computed_field(
    ...
    *,
    ...
    def dec(f: Any) -> Any:
        ...
        # if the function isn't already decorated with `@property` (or another
        # descriptor), then we wrap it now
