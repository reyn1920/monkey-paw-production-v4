...
def create_model_from_typeddict(
    # Mypy bug: `Type[TypedDict]` is resolved as `Any`
    ...
    **kwargs: Any,
    ...
    # Best case scenario: with python 3.9+ or when `TypedDict` is imported
...
def create_model_from_namedtuple(
    ...
    to have type `Any`.
    """
    # With python 3.10+, `__annotations__` always exists but can be empty
    # hence the `getattr... or...` logic
