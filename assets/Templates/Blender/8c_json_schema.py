...
class _DefinitionsRemapping:
    ...
    def from_prioritized_choices(
        ...
        for _iter in range(
            ...
            # For every possible remapped DefsRef, collect all schemas that
            # that DefsRef might be used for:
            ...
            # if it introduces no ambiguity, i.e., there is only one distinct
            # schema for that DefsRef.
            ...
            for original_defs_ref in definitions:
                ...
                # Pick the first alternative that has only one schema, since
    ...
    def remap_json_schema(self, schema: Any) -> Any:
        ...
        if isinstance(schema, str):
            # Note: this may not really be a JsonRef; we rely on having no
            # collisions between JsonRefs and other strings
...
class GenerateJsonSchema:
    ...
    # this value can be modified on subclasses to easily control which
    ...
    def __init__(self, by_alias: bool = True, ref_template: str = DEFAULT_REF_TEMPLATE):
        ...
        # definitions, which would also work better in a generated OpenAPI
        # client, etc.
        ...
        for key in core_schema_types:
            ...
            try:
                ...
                if os.getenv("PYDANTIC_PRIVATE_ALLOW_UNHANDLED_SCHEMA_TYPES"):
                    continue
                raise TypeError(
                    f"No method for generating JsonSchema for core_schema.type={
                        key!r} "
                    f"(expected: {
    ...
    def generate_inner(
        ...
        def handler_func(schema_or_field: CoreSchemaOrField) -> JsonSchemaValue:
            ...
            if self.mode == "serialization" and "serialization" in schema_or_field:
                ...
                # This is only relevant for `nullable` schemas though, so we
                # special case here.
                if (
        ...
        # TODO: I dislike that we have to wrap these basic dict updates in
        # callables, is there any way around this?
        ...
        if js_extra := metadata.get("pydantic_js_extra"):
            ...
            def js_extra_handler_func(
                ...
                if isinstance(js_extra, dict):
                    ...
                    # similar to typing issue in _update_class_schema when
                    # we're working with callable js extra
    ...
    def is_subclass_schema(
        ...
        Returns:
            The generated JSON schema.
        """
        # Note: This is for compatibility with V1; you can override if you want
        # different behavior.
        return {}
    ...
    def tuple_schema(self, schema: core_schema.TupleSchema) -> JsonSchemaValue:
        ...
        if "variadic_item_index" in schema:
            ...
            if variadic_item_index + 1 == len(schema["items_schema"]):
                # if the variadic item is the last item, then represent it
                # faithfully
    ...
    def dict_schema(self, schema: core_schema.DictSchema) -> JsonSchemaValue:
        ...
        if "$ref" not in keys_schema:
            ...
            # Note that we avoid calling `self.resolve_ref_schema`, as it might
            # not exist yet.
            keys_pattern = None
        ...
        if (
            ...
            # If this is a definition reference schema, it most likely has
            # constraints:
    ...
    def default_schema(self, schema: core_schema.WithDefaultSchema) -> JsonSchemaValue:
        ...
        if (
            ...
            try:
                ...
                # serializes as an int. In this case, `ser_func` is `int` and
                # `int('1MB')` fails).
                self.emit_warning(
                    "non-serializable-default",
                    f"Unable to serialize value {
                        default!r} with the plain serializer; excluding default from JSON schema",
                )
                return json_schema
    ...
    def nullable_schema(self, schema: core_schema.NullableSchema) -> JsonSchemaValue:
        ...
        if inner_json_schema == null_schema:
            ...
            # I'll use 'anyOf' for now, but it could be changed it if it would
            # work better with some external tooling
    ...
    def tagged_union_schema(
        ...
        # This reflects the v1 behavior; TODO: we should make it possible to
        # exclude OpenAPI stuff from the JSON schema
    ...
    def _extract_discriminator(
        ...
        if isinstance(schema["discriminator"], list):
            # If the discriminator is a single item list containing a string,
            # that is equivalent to the string case
            ...
            # property on all choices. If so, that property will be used as the
        ...
        if isinstance(alias, str):
            ...
            for path in alias:
                if (
                    ...
                    # should ensure the first such item is what belongs in the
                    # JSON schema
                    name = path[0]
                    break
        else:
        ...
        * deprecated
        ...
        if (config_title := config.get("title")) is not None:
            ...
            if not isinstance(title, str):
                raise TypeError(
                    f"model_title_generator {model_title_generator} must return str, not {
                        title.__class__}"
                )
        ...
        # BaseModel and dataclasses; don't use cls.__doc__ as it will contain
        # the verbose class signature by default
        docstring = (
        ...
        if isinstance(json_schema_extra, (staticmethod, classmethod)):
            # In older versions of python, this is necessary to ensure
        ...
        if isinstance(json_schema_extra, dict):
            ...
            # FIXME: why are there type ignores here? We support two signatures
            # for json_schema_extra callables...
    ...
    def dataclass_schema(self, schema: core_schema.DataclassSchema) -> JsonSchemaValue:
        ...
        if is_builtin_dataclass(cls):
            # vanilla dataclass; don't use cls.__doc__ as it will contain the
            # class signature by default
            description = None
        else:
    ...
    def multi_host_url_schema(
        ...
        Returns:
            The generated JSON schema.
        """
        # Note: 'multi-host-uri' is a custom/pydantic-specific format, not part
    ...
    def field_title_should_be_set(self, schema: CoreSchemaOrField) -> bool:
        ...
        if _core_utils.is_core_schema_field(schema):
            ...
            if schema["type"] in {"default", "nullable", "definitions"}:
                return self.field_title_should_be_set(
                    schema["schema"]
                )  # type: ignore[typeddict-item]
    ...
    def get_defs_ref(self, core_mode_ref: CoreModeRef) -> DefsRef:
        ...
        Returns:
            The definitions key.
        """
        # Split the core ref into "components"; generic origins and arguments
    ...
    def handle_ref_overrides(self, json_schema: JsonSchemaValue) -> JsonSchemaValue:
        ...
        if "$ref" in json_schema:
            # prevent modifications to the input; this copy may be safe to drop
            # if there is significant overhead
...
class WithJsonSchema:
    ...
    def __get_pydantic_json_schema__(
        ...
        if self.json_schema is None:
            ...
            # pydantic.json_schema.GenerateJsonSchema._named_required_fields_schema
            raise PydanticOmit
        else:
        ...
        if isinstance(current, dict):
            for key, value in current.items():
                if key == "examples" and isinstance(value, list):
                    ...
                    # could be found, but would require more advanced JSON
