    ...
    def __getattr__(self, item: str) -> Any:
        try:
            ...
            raise AttributeError(
                f"type object '{
                    self.__name__}' has no attribute {exc}"
            ) from exc
    ...
    def __getattr__(self, item: str) -> Any:
        try:
            ...
            try:
                ...
                # re-raising changes the displayed text to reflect that `self`
                # is not a type
...
class _ExtraMeta(type):
    def __getattribute__(self, __name: str) -> Any:
        # The @deprecated decorator accesses other attributes, so we only emit
        # a warning for the expected ones
