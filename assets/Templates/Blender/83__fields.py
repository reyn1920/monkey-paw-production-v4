    ...
    *,
    ...
    for ann_name, (ann_type, evaluated) in type_hints.items():
        if ann_name == "model_config":
            ...
            # protected namespaces (where `model_config` might be allowed as a
            # field name)
            continue
                ...
                for b in bases:
                    if hasattr(b, ann_name):
                        if not (
                            ...
                            raise NameError(
                                f'Field "{ann_name}" conflicts with member {
                                    getattr(
                                        b, ann_name)}'
        ...
        for base in bases:
            ...
            if hasattr(base, ann_name):
                if base is generic_origin:
                    # Don't warn when "shadowing" of attributes in parametrized
                    # generics
                    continue
                ...
                if ann_name not in annotations:
                    # Don't warn when a field exists in a parent class but has
                    # not been defined in the current class
                    continue

                warnings.warn(
                    f'Field name "{ann_name}" in "{
        ...
        if (
            ...
            if ann_name in annotations or ann_name not in parent_fields_lookup:
                ...
                # Nothing stops us from just creating a `FieldInfo` for this
                # type hint, so we do this.
                ...
                # copy the field to make sure typevar substitutions don't cause
                # issues with the base classes
                field_info = copy(parent_fields_lookup[ann_name])

        # An assigned value is present (either the default value, or a
        # `Field()` function)
        else:
            ...
            # The `from_annotated_attribute()` call below mutates the assigned
            # `Field()`, so make a copy:
            original_assignment = (
            ...
            # parameterized with an unknown forward reference (and we'll need
            # it to rebuild the field info):
            ...
            if not evaluated:
                ...
                warnings.warn(
                    f"Annotation {
                        ann_name!r} is marked as final and has a default value. Pydantic treats {
                        ann_name!r} as a "
                    "class variable, but it will be considered as a normal field in V3 to be aligned with dataclasses. If you "
                    f"still want {
                        ann_name!r} to be considered as a class variable, annotate it as: `ClassVar[<type>] = <default>.`",
                    category=PydanticDeprecatedSince211,
                    # Incorrect when `create_model` is used, but the chance
                    # that final with a default is used is low in that case:
                    stacklevel=4,
                )
                class_vars.add(ann_name)
                continue
        ...
        # to make sure the decorators have already been built for this exact
        # class
    ...
    with ns_resolver.push(cls):
        for f_name, field_info in cls.__pydantic_fields__.items():
            if field_info._complete:
                ...
                # The description might come from the docstring if
                # `use_attribute_docstrings` was `True`:
