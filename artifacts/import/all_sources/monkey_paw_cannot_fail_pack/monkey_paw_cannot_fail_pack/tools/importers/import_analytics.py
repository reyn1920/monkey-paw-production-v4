...
import csv, sys, sqlite3, hashlib
...
SCHEMA = {
    ...
    "thumbnails": "create table if not exists thumbnails (id integer primary key, hash text unique, filename text, path text)",
}
...
def import_csv(con, path: Path):
    ...
    if "youtube" in name or "yt" in name:
        # Expect columns: date,views,watch_hours,subs,revenue_cents
        ...
        # Expect columns: date,subs,opens,clicks
        for row in csv.DictReader(path.open()):
...
def main(folder: str):
    ...
    for f in p.glob("*.csv"):
        import_csv(con, f)
    con.close()
    print(f"Imported CSVs from {folder} into {DB}")

if __name__ == "__main__":
    main(sys.argv[1] if len(sys.argv)>1 else str(BASE/"reports"))
