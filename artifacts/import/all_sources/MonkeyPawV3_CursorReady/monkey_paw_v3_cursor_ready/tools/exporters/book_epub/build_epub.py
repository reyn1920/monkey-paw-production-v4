...
def build_epub(inputs_glob: str, out_file: str):
    ...
    if not chapter_files:
        c = Path("artifacts/staging/blank.txt"); c.parent.mkdir(parents=True, exist_ok=True); c.write_text("Blank chapter — add drafts to artifacts/staging"); chapter_files = [str(c)]
    chapters = []
    for i, path in enumerate(chapter_files, 1):
        c = epub.EpubHtml(title=f"Chapter {i}", file_name=f"chap_{i}.xhtml", lang="en")
        c.content = f"<h1>Chapter {i}</h1><pre>{Path(path).read_text()}</pre>"; book.add_item(c); chapters.append(c)
    book.toc = tuple(chapters); book.add_item(epub.EpubNcx()); book.add_item(epub.EpubNav()); book.spine = ['nav'] + chapters
    Path(out_file).parent.mkdir(parents=True, exist_ok=True); epub.write_epub(out_file, book)
if __name__ == "__main__":
    ...
    build_epub(ins, out); print(f"Wrote {out}")
