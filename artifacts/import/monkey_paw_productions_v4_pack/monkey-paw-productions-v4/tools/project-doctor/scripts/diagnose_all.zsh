#!/bin/zsh
set -euo pipefail
ROOT="${1:-.}"; TS=$(date +"%Y%m%d_%H%M%S"); OUT="${ROOT}/reports/diagnostic_${TS}"
mkdir -p "${OUT}"
python3 - <<'PY'
...
for py in ROOT.rglob("*.py"):
    ...
    for m in decor_pat.finditer(text):
        deco_prefix = m.group(1); method = (m.group(2) or "ANY").upper(); path = m.group(3)
        idx = m.end(); sub = text[idx:idx+1000]
        import re as _r; mdef = def_pat.search(sub); func = mdef.group(1) if mdef else "<unknown>"
        line = text[:m.start()].count("\n") + 1
        routes.append({"file": str(py.relative_to(ROOT)), "line": line, "deco": deco_prefix, "method": method, "path": path, "function": func})
(pathlib.Path(f"{OUT}/routes_static.json")).write_text(json.dumps({"routes": routes}, indent=2))
PY
